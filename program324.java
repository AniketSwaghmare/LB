...
class DBMS {
    ...
    // Insert into student Aniket 1000;
    // select * from student
    public void StartDBMS() {
        Scanner scanobj = new Scanner(System.in);

        System.out.println("Aniket customised DBMS started succesfully....");
        String Query = "";
        ...
        while (true) {
            ...
            if (QuerySize == 1) {
                if ("Help".equals(tokens[0])) {
                    ...
                    System.out.println("Specific RID Info Display : select * from student where _RID ");
                    System.out.println("Specific RID Info Delete  : Delete where _RID");
                    System.out.println("----------------------------------------------------------------");
                } else if ("Exit".equals(tokens[0])) {
                    System.out.println("Thank you for using Aniket DBMS");
                    break;
                }
            } else if (QuerySize == 2) {
            ...
            // select * from student where _Name
            else if (QuerySize == 6) {
                if ("select".equals(tokens[0])) {
                    if ("where".equals(tokens[4])) {
                        DisplaySpecific(tokens[5]);
                    }
                }
            }

            else if (QuerySize == 6) { // rahil ahi
                if ("select".equals(tokens[0])) {
                    if ("where".equals(tokens[4])) {
                        DisplaySpecific(Integer.parseInt(tokens[5]));
                    }
                }
            }
